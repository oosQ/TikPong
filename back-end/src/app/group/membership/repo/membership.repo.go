...
func ListMembers(groupID, cursor string, limit int) (*dto.ListMembersResponse, error) {
	rows, err := database.DB.Query(`
		SELECT gm.user_id, u.nickname, u.avatar_path, COALESCE(u.status, 'offline'), gm.role
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.UserID, &item.Nickname, &item.AvatarPath, &item.Status, &item.Role); err != nil {
			return nil, err
		}
