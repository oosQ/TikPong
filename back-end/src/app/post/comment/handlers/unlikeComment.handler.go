...
func UnlikeCommentHandler(w http.ResponseWriter, r *http.Request) {
	...
	err := services.UnlikeComment(userCtx.ID, commentID)
	if err != nil {
		utils.SendError(w, err.Error(), http.StatusBadRequest)
		return
	}

	utils.SendSuccess(w, dto.CommentActionResponse{CommentID: commentID, Action: "unliked"}, "Comment unliked successfully")
}
