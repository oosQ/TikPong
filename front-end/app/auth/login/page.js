  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/sessions`, {
        ...
        credentials: "include",
        body: JSON.stringify({
          nickname_or_email: nicknameOrEmail,
          password,
        }),
      });
      ...
      <div className="w-full max-w-xl rounded-[32px] border border-white/10 bg-white/[0.03] p-8 shadow-[0_24px_60px_rgba(0,0,0,0.45)] backdrop-blur sm:p-10">
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-white/55">
            Sign in with your nickname or email and password.
          </p>
          <p className="mt-4 text-sm text-white/50">
            No account?{" "}
            <Link href="/auth/register" className="font-semibold text-white transition hover:text-[#fe2c55]">
              Create one
            </Link>
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={(event) => setNicknameOrEmail(event.target.value)}
          ...
          <div>
            ...
            <div className="mt-3 text-right">
              <Link
                ...
                className="text-sm font-semibold text-white/72 transition hover:text-[#fe2c55]"
              >
                Forgot password?
              </Link>
            </div>
